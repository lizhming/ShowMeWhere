...
class ViewSelected extends Component {
  ...
  renderScene(route, navigator) {
    ...
    var langValue = <Text style={styles.languageValue}>{language.name}</Text>;
    if(language.english == 'Bengali'||language.english == 'Urdu'||language.english == 'Arabic'){
      langValue = <Text style={styles.specialValue}>{language.name}</Text>;
    }
  ...
  _renderFeeling(data) {
    ...
    return (
      <View style={styles.row}>
        <TouchableOpacity
            activeOpacity={0.5}
            style={{flex: 1, flexDirection: 'row', justifyContent: 'space-between', alignSelf: 'stretch'}}
  ...
  _selectFeeling(data) {
    ...
    if(id!=='pain'){
      AlertIOS.alert(
        this.state.words.alerts.remove + ', "' + data.english + '"?',
        '',
        [
          {text: 'Cancel'},
      ...
      <View style={styles.row}>
        <TouchableOpacity
          ...
          <View style={styles.imageContainer}>
            <Image style={styles.image} resizeMode={Image.resizeMode.contain} source={image}/>
          </View>
        </TouchableOpacity>
      </View>
    )
